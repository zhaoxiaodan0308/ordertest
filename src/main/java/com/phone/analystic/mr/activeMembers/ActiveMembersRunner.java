 ...
 * Description:统计用户表和浏览器表的活跃会员
 *             pv事件的memberID去重
 * History:
 * <author> <time> <version> <desc>
 * 作者姓名 修改时间 版本号 描述
 */
public class ActiveMembersRunner implements Tool {
    private static Logger logger = Logger.getLogger(ActiveMembersRunner.class);
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "activeMembers to mysql");

        //设置运行的类
        job.setJarByClass(ActiveMembersRunner.class);

        //设置map和输出
        job.setMapperClass(ActiveMembersMapper.class);
        job.setMapOutputKeyClass(StatsUserDimension.class);
        job.setMapOutputValueClass(TimeOutputValue.class);

        //设置reduce和输出
        job.setReducerClass(ActiveMembersReducer.class);
        job.setMapOutputKeyClass(StatsUserDimension.class);
    ...
    public static void main(String[] args) {
        try {
            ToolRunner.run(new Configuration(), new ActiveMembersRunner(), args);
        } catch (Exception e) {
            logger.error("运行异常");
        }
    }
